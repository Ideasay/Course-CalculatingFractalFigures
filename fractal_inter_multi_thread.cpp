...
#include <vector>
#include <string>
const int THREAD_NUM = 16; //线程数目，最佳为16线程
int main() {
	Bmp256 bmp{ W, H };

	auto t1 = steady_clock::now();

	std::vector<std::thread> threads; //容器
	int Wi = W / THREAD_NUM;//对宽(W)进行线程分割
	int A = THREAD_NUM;
	std::cout << "Start Thread_id:\n ";
	for (int a = 0; a < A; a++) {//对线程数目进行遍历
		threads.push_back(std::thread(
			[&](int th_id) {
				std::cout << th_id << "\n"  << std::endl; //显示启动的线程

				for (int j = 0; j < H; ++j)
					for (int i = th_id; i < W; i += A)
						bmp(j, i) = Mandelbrot(complex{ RMIN + i / RESN, IMIN + j / RESN }); //进行操作

				std::cout << "END:" << th_id << "\n" << std::endl; //输出结束关闭的线程
			}, a));
	}

	for (auto& th : threads) { //数据合并
		th.join();
	}
